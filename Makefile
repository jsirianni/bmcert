...
build: clean
	...
	@docker create -ti --name bmcertartifacts bmcert:${VERSION} bash && \
	    docker cp bmcertartifacts:/bmcert-v${VERSION}-linux-amd64.zip artifacts/bmcert-v${VERSION}-linux-amd64.zip && \
	    docker cp bmcertartifacts:/bmcert-v${VERSION}-darwin-amd64.zip artifacts/bmcert-v${VERSION}-darwin-amd64.zip && \
	    docker cp bmcertartifacts:/bmcert-v${VERSION}.SHA256SUMS artifacts/bmcert-v${VERSION}.SHA256SUMS
